...
function handle_ipn($vars) {
	...
	$user_login = $vars['custom'];				// Login id of person paying
	$user = get_userdatabylogin($user_login);
	$user_email = $user->user_email;
	$IPN_payer_email = $vars['payer_email'];
	if($IPN_reference and $IPN_address and $payment_status and $user_login) {
		if($item_name=="Holding Deposit") {
			...
			while ( $loop->have_posts() ) : $loop->the_post();
				...
				if($address==$IPN_address) {
					...
					if($payment_status=="Completed") {
						...
						add_post_meta($the_ID, 'ipn_payer_email', $user_login, true) or update_post_meta($the_ID, 'ipn_payer_email', $user_login);
						add_post_meta($the_ID, 'ipn_all', $IPN_all, true) or update_post_meta($the_ID, 'ipn_all', $IPN_all);
						update_post_meta($the_ID, 'ipn_datetime', time());
						// add payer to their own group
						update_user_meta($user->ID, 'group', $user_login);
						// email the deposit payer
						$email = $user_email.",glenn@sweetspot.com"; // TO-DO: get the manager email address for a property dynamically
						...
						// mail everyone else who is an applicant on this property so they know it is on offer
						$tmpPost = $post;
						$viewings_loop = new WP_Query('caller_get_posts=1&post_type=viewings&post_status=publish');
						while ($viewings_loop->have_posts() ) : $viewings_loop->the_post();
							$viewing_address = get_post_meta($post->ID, 'address', true);
							if($viewing_address==$IPN_address) { // only look at viewings for this property
								$viewing_booker = get_the_author_meta('user_email');
								if($user_email!=$viewing_booker) { // don't email the person paying the holding deposit
									$viewing_date = get_post_meta($post->ID, 'datetime', true);
									$under_offer_subject = "{$address} is now under offer";
									$under_offer_text = "Hi there!\n\nI'm writing to you because you have a viewing booked on {$address} for {$viewing_date}. I'm afraid the property is now under offer. If the group who have paid the holding deposit don't take the property, it will come back on the market and we'll be back in touch.";
									sweetspot_mail($viewing_booker, $under_offer_subject, $under_offer_text);
								}
							}
						endwhile;
						$post = $tmpPost;
						if ($IPN_reference=="Cash") {
							...
							echo "<html><head><meta http-equiv=\"Refresh\" content=\"5; url={$dash_url}\" /></head><body><p>Cash payment acknowledged.</p><p>Redirecting back to the dash, click to go <a href=\"{$dash_url}\">back to the dash</a> if you are not redirected within 5 seconds.</body></html>";
						} else {
			...
			if($payment_status=="Completed") {
				...
				if($all_paid) {
					...
					sweetspot_mail('glenn@sweetspot.com', "All security deposits paid for {$IPN_address}", $email_text, true);
				}
				echo "Payment acknowledged";
			} else if ($payment_status=="Refunded") {
...
function propertiesColumns($columns) {
	$columns['holding_deposit'] = 'Holding deposit ref';
	return $columns;
}
...
function makePayPalLink($id) { // JRL: TO-DO: I want to make this link directly to the transaction or refund page, but I haven't figured out how to yet.
	return '<a href="https://www.paypal.com/uk/cgi-bin/webscr?cmd=_history">'.$id."</a>";
}

// adds properties reset panel to a Properties page
/* removed in Phase VII since reset is happening through the admin dash 
...
function my_show_extra_profile_fields( $user ) { ?>
	...
	<table class="form-table">
		...
		<tr>
			<th><label for="submitted_to_paragon">Sent on to Paragon</label></th>
			<td>
				<input type="text" name="submitted_to_paragon" id="submitted_to_paragon" value="<?php echo esc_attr( get_the_author_meta( 'submitted_to_paragon', $user->ID ) ); ?>" class="regular-text" />
			</td>
		</tr>
	</table>
<?php }
...
function my_save_extra_profile_fields( $user_id ) {
	...
	update_user_meta( $user_id, 'submitted_to_paragon', $_POST['submitted_to_paragon'] );
}
 ...
 function processViewingBooking($vars) {
	...
	if($username && $address && $datetime) {
		...
		if ( !$user_id ) {
			...
			$name_parts = explode(' ',$booked_by_name);
...
function sweetspot_mail($to_address, $subject, $body, $no_reply=FALSE) {
	...
	if($no_reply) {
		...
		$headers .= "From: SweetSpot <no-reply@sweetspot.com>\r\n";
	} else {
		$headers .= "Reply-To: glenn@sweetspot.com\r\n";
		$headers .= "From: SweetSpot <glenn@sweetspot.com>\r\n";
	}
...
// *********** Function to compare two dates (only necessary when using php 5.2) ************
...
// ********** Function to reset property **********

function reset_property($the_ID) {
	global $wpdb;
	// tell any active group that we are releasing the property
	$address = get_post_meta($the_ID, '_address', true);
	$lead_tenant_email = get_post_meta($the_ID, 'ipn_payer_email', true);
	$reset_email_subject = "We've put {$address} back on the market";
	$reset_email_text = "Hi.\n\nFor one reason or another, we've put {$address} back on the market, which means you no longer have a hold over the property. I'm sorry it didn't work out this time.";
	$peopleIDs = $wpdb->get_col( $wpdb->prepare(
		"SELECT $wpdb->users.ID FROM $wpdb->users"
	));
	$persons = array();
	foreach($peopleIDs as $personID) {
		...
		if($person->user_email == $lead_tenant_email) {
			$lead_tenant = $person;
		}
	}
	if($lead_tenant) {
		...
		foreach($persons as $person) {
			if($person->group==$group) {
				sweetspot_mail($person->user_email, $reset_email_subject, $reset_email_text);
			}
		}
		
		// clear up the property
		...
		delete_post_meta($the_ID, 'ipn_datetime');
	}
}

// *********** Function to check whether all applicants have completed their steps *******

function checkAllApplicants($propertyID) { 
	if(!get_post_meta($propertyID, 'notified_manager_about_AST', true)) { // only do this if the manager hasn't already been notified
		global $current_user;
		global $wpdb;
		...
		if(!$lead_tenant) { // don't bother if there's no lead tenant
			return;
		}
		...
		foreach ($peopleIDs as $personID) {
			$person = get_userdata($personID);
			if($person->group==$group) {
				if($person->group==$person->user_login) { // person is lead tenant
		...
		if($all_paid) {
			...
			$manager_email_subject = "All tenants approved for {$paid_holding_deposit_address}"; // TO-DO: get into an email admin plugin
			$manager_email_text = "All the tenants have completed their approval steps for {$paid_holding_deposit_address}.\n\nPlease get their tenancy agreement together and send it out."; // TO-DO: get into an email admin plugin
